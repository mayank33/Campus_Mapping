...
var app = {
	...
	var map = L.map('map-canvas').setView([17.4453593878408,78.3509433037455 ], 18);
	map1 = map;
...
function overlay_points_one(set_of_points,dist)
{
	...
	{
		x=0;
                put_marker(set_of_points[0][1],set_of_points[0][2],"","",set_of_points[0][0]);
		while(set_of_points[x+1][1]!=-1)
		{
			...
			x=x+1;
                ...
                put_marker(set_of_points[x][1],set_of_points[x][2],"","",set_of_points[x][0]);		
...
function put_marker(lat,long,image,desc,msg)
{
        if( image != "")
        {
                var imagepos = "<img src\= "+image+" width\= \"100\" height\= \"100\"/>";
                L.marker([lat,long]).addTo(map1).bindPopup(msg+'<br/><p>'+imagepos +'</p>'+desc).openPopup();
        }
        else
        {
                L.marker([lat,long]).addTo(map1).bindPopup(msg+'<br/>'+desc).openPopup();
        }
...
function search_by_building(name,building_file_name)
{
    ...
    $.ajax({'async': false,
        ...
        if(flag==1)
        {
            put_marker(lat,long,image,desc,room);
        }
        else
        {
            return -1;
...
function search_by_lab(name,room_file_name)
{
    if(name=="")
        return -1;
    ...
    $.ajax({'async': false,
        ...
        'success': function(data)
	   {
    		$.each(data,function(i,x)
    		{
    			if(x.room.toLowerCase()==name.toLowerCase())
    			{
    				flag=1;
    				lat=x.lat;
    				long=x.long;
    	...
    	if(flag==1)
    	{
    		put_marker(lat,long,image,desc,"Room : "+room+" Building : "+building);
    	}
    	else
    	{
            return -1;
